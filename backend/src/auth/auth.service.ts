...
export class AuthService {
  ...
  async login(loginUserDto: LoginUserDto) {
    ...
    const user = await this.userModel
      .findOne({ email }, { email: 1, password: 1, _id: 1, entidad: 1, roles: 1, name: 1, lastname: 1 });
    if (!user)
  ...
  async sendActivationCodeToEmail(emailDto: EmailDto) {
    ...
    if (user) {
      throw new Error(`Usuario con el email ${email} ya existe en la base de datos`);
    }
    try {
      ...
      const emailToUserDto: EmailToUserDto = {
        ...
        title: 'Has solicitado registrarte por correo en Karbono. Si no fue así ignora este correo',
  ...
  async registerByActivationCode(createUserByEmailDto: CreateUserByEmailDto) {
    ...
    let user = await this.userModel.findOne({ email });
    if (!user) {
      throw new NotFoundException(`Usuario con el email ${email} no encontrado en la DB`);
    }
    if (user.verificationCode !== activationCode) {
      throw new NotFoundException('El código de activación proporcionado es incorrecto')
    }
    try {
      let userToUpdate = {
        verificationCode: '',
  ...
  async sendVerificationCode(emailDto: EmailDto) {
    ...
    try {
      ...
      const emailToUserDto: EmailToUserDto = {
        email,
        subject: 'Olvidó contraseña de QPAlliance. Este es su código de verificación',
        title: 'Has solicitado cambiar la contraseña de QPAlliance. Si no es así ignora este correo',
        message: verificationCode,
  ...
  // -----------------------------------------------------
  async verifyCodeAndUpdatePassword(verifyCodeAndUpdatePassword: VerifyCodeAndUpdatePasswordDto) {
    let { email, password, verificationCode } = verifyCodeAndUpdatePassword;
    let user = await this.userModel.findOne({ email });
    if (!user) {
      throw new NotFoundException(`Usuario con el email ${email} y código empleados no encontrado en la DB`);
    }
    ...
    try {
      ...
      console.log(error)
      throw new InternalServerErrorException(`No se puede actualizar la contraseña. Error interno del servidor`)
    }
  ...
  async acceptInvitation(token: string) {
    ...
    let userInvited = await this.userModel
      .findById(id)
    if (!userInvited) {
  ...
  async registerByInvitation(registerByInvitationDto: RegisterByInvitationDto) {
    ...
    try {
      ...
      console.log(error);
      throw new InternalServerErrorException(`No se puede actualizar el usuario. Error interno del servidor ${error}`)
    }
  }
  // -----------------------------------------------------
  async byRol(rol: string) {
    let usersByRol = await this.userModel.find({ roles: { $in: rol } }, { _id: 1, name: 1, lastname: 1 });
    return usersByRol
  }
  // -----------------------------------------------------
  ...
  async update(user: IUser, updateUserDto: UpdateUserDto) {
    let userToUpdate = await this.userModel.findOne({ _id: user.id, isActive: true });
  ...
  async getMyGroup(user: IUser) {
    const myGroup = await this.userModel.find({ group_admin: user.email, isActive: true })
    return myGroup
  }
  // -----------------------------------------------------
  async updateGroup(groupDto: GroupDto) {
    ...
    try {
      ...
      console.log(error);
      throw new InternalServerErrorException(`No se pudo actualizar. Error interno del servidor`)
    }
  ...
  async removeMember(email: string) {
    ...
    try {
      ...
      console.log(error);
      throw new InternalServerErrorException(`No se pudo remover el usuario de su equipo. Error interno del servidor`)
    }
  ...
  private handleExceptions(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(`El usuario con ${JSON.stringify(error.keyValue)} ya existe en la base de datos`);
    }
    console.log(error);
    throw new InternalServerErrorException(`No se puede crear el usuario. Error interno del servidor`);
  }
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
