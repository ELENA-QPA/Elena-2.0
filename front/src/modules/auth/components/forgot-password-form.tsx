...
import { ForgotPasswordSuccessResponse, ErrorResponse } from "@/modules/auth/data/interfaces/auth-responses";
...
export const ForgotPasswordForm = () => {
  ...
  async function onSubmit(values: { email: string }) {
    try {
      ...
      const resp = await authRepository.forgotPassword(values);

      if ("email" in resp) {
        toast.success(resp.message, { position: "top-right", closeButton: true });
        setErrorMessage("");
        router.push("/auth/reset-password"); // Redirige a restablecer contraseña
      } else {
  ...
  return (
    <Card className="w-full max-w-[380px] rounded-2xl py-4 shadow-lg border-0 bg-white">
      <div className="flex flex-col justify-center place-items-center">
        <div className="text-gray-800 font-sans text-3xl font-semibold pt-4">Recuperar contraseña</div>
        <div className="text-gray-500 text-sm mt-2 mb-4">Ingresa tu correo para recibir el código</div>
      </div>
      <CardContent>
        <Form {...form}>
