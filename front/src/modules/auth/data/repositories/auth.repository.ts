...
export abstract class AuthRepository {
  ...
  abstract acceptInvitation(
    token: string
  ): Promise<AcceptInvitationSuccessResponse | ErrorResponse>;
  abstract forgotPassword(data: {
    email: string;
  }): Promise<ForgotPasswordSuccessResponse | ErrorResponse>;
  abstract sendActivationCodeToEmail(data: {
    email: string;
  }): Promise<SendActivationCodeSuccessResponse | ErrorResponse>;
  abstract inviteUser(data: {
    name: string;
    lastname: string;
    email: string;
    phone: string;
    roles: string[];
  }): Promise<InviteUserSuccessResponse | ErrorResponse>;
  abstract registerByActivationCode(data: {
    name: string;
    lastname: string;
    email: string;
    phone: string;
    ...
    roles: string[];
    he_leido: boolean;
  }): Promise<RegisterByActivationCodeSuccessResponse | ErrorResponse>;
  abstract registerByInvitation(data: {
    id?: string;
    token: string;
    email: string;
    name: string;
  ...
  abstract verifyCodeAndUpdatePassword(data: {
    email: string;
    verificationCode: string;
    password: string;
  }): Promise<VerifyCodeSuccessResponse | ErrorResponse>;
}
...
export class AuthRepositoryImpl implements AuthRepository {
  ...
  async register(data: {
    ...
    try {
      ...
      if (axiosRequest.statusCode === HttpStatusCode.created) {
        return {
          message:
            axiosRequest.body.message || "Usuario registrado exitosamente",
  ...
  async forgotPassword(data: {
    ...
    try {
      ...
      if (
        ...
        return {
          message:
            typeof axiosRequest.body === "string"
              ? axiosRequest.body
  ...
  async sendActivationCodeToEmail(data: {
    email: string;
  }): Promise<SendActivationCodeSuccessResponse | ErrorResponse> {
    try {
      const axiosRequest = await this.httpClient.request({
      ...
      if (axiosRequest.statusCode === HttpStatusCode.ok) {
        return {
          message:
            axiosRequest.body.message ||
            "Código de activación enviado al email",
        ...
        return {
          statusCode: axiosRequest.statusCode,
          message:
            axiosRequest.body.message || "Error al enviar código de activación",
          error: axiosRequest.body.error || "Unknown Error",
        };
      }
    } catch (error: any) {
  ...
  async inviteUser(data: {
    ...
    try {
      ...
      if (
        ...
        const message = "Invitación enviada exitosamente";
  ...
  async registerByActivationCode(data: {
    name: string;
    lastname: string;
    email: string;
    phone: string;
    ...
    try {
      ...
      if (axiosRequest.statusCode === HttpStatusCode.created) {
        ...
        return {
          statusCode: axiosRequest.statusCode,
          message:
            axiosRequest.body.message || "Error al registrar por activación",
          error: axiosRequest.body.error || "Unknown Error",
        };
      }
    } catch (error: any) {
  ...
  async registerByInvitation(data: {
    id?: string;
    token: string;
    email: string;
    name: string;
    ...
    try {
      ...
      if (axiosRequest.statusCode === HttpStatusCode.created) {
        return {
          message:
            axiosRequest.body.message ||
            "Usuario registrado por invitación exitosamente",
          user: axiosRequest.body.user,
          token: axiosRequest.body.token,
        };
      } else {
        return {
          statusCode: axiosRequest.statusCode,
          message:
            axiosRequest.body.message || "Error al registrar por invitación",
          error: axiosRequest.body.error || "Unknown Error",
        };
      }
    } catch (error: any) {
  ...
  async verifyCodeAndUpdatePassword(data: {
    email: string;
    verificationCode: string;
    password: string;
  }): Promise<VerifyCodeSuccessResponse | ErrorResponse> {
    try {
      ...
      if (
        ...
        return {
          message:
            axiosRequest.body.message || "Contraseña actualizada exitosamente",
          success: true,
        };
      } else {
        return {
          statusCode: axiosRequest.statusCode,
          message:
            axiosRequest.body.message || "Error al actualizar contraseña",
          error: axiosRequest.body.error || "Unknown Error",
        };
      }
    } catch (error: any) {
