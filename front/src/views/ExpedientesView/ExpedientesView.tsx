...
import { Search, Plus, Filter, Eye, Edit, Loader2, RefreshCw, Trash2, ChevronLeft, ChevronRight, Calendar, ChevronDown, ChevronUp } from "lucide-react"
...
export default function ExpedientesView({ initialCasos = [], initialTotal = 0, initialError = null }: ExpedientesViewProps) {
  ...
  const compareNames = useCallback((name1: string, name2: string) => {
    ...
    const cityVariations: { [key: string]: string[] } = {
      "bogota": ["bogota d.c.", "bogota dc", "distrito capital", "santa fe de bogota"],
  ...
  const filteredCities = useMemo(() => {
    ...
    const cities = allCities.filter(city => city.departamento === departmentFilter);
  ...
  useEffect(() => {
    ...
    if (despachoJudicial && despachoJudicial.length > 0) {
      ...
      if (cityDespachos && Array.isArray(cityDespachos) && cityDespachos.length > 0) {
  ...
  const checkAuth = () => {
    ...
    if (!token && !user) {
      console.error("[EXPEDIENTES][Auth Error]: No hay cookies de autenticación");
      setAuthStatus('unauthenticated');
      return false;
    }

    console.log("[EXPEDIENTES][Auth Success]: Cookies de autenticación encontradas");
    setAuthStatus('authenticated');
    return true;
  }

  // Función para cargar más datos del servidor (en lotes de 20)
  const loadMoreFromServer = useCallback(async (batchSize: number = 20) => {
    if (isLoadingMore || !hasMoreToLoad) return;
    
    setIsLoadingMore(true);
    try {
      const currentOffset = allLoadedCasos.length;
      console.log("[EXPEDIENTES][Load More Server]:", { currentOffset, batchSize });
      ...
      if ('records' in response) {
        ...
        if (newCasos.length === 0) {
          ...
          const uniqueNewCasos = newCasos.filter(caso => !existingIds.has(caso._id));
          ...
          if (newCasos.length < batchSize) {
            setHasMoreToLoad(false);
            console.log("[EXPEDIENTES][End Detected]: Último lote incompleto, fin de datos");
          }
        }
  ...
  const handleDeleteCaso = useCallback(async (id: string) => {
  ...
  useEffect(() => {
    ...
    if (initialError) {
      console.log("[EXPEDIENTES][Server Error]: Error del servidor", initialError);
  ...
  const filteredCasos = useMemo(() => {
    ...
    if (estadoFilter !== "all") {
      console.log("[EXPEDIENTES][Estado Filter]:", {
        ...
        sampleEstados: filtered.slice(0, 5).map(c => ({ id: c._id, estado: c.estado }))
      });
      filtered = filtered.filter(caso => caso.estado === estadoFilter);
      console.log("[EXPEDIENTES][Estado Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, estado: c.estado }))
      });
    }
    ...
    if (clientTypeFilter !== "all") {
      console.log("[EXPEDIENTES][ClientType Filter]:", {
        clientTypeFilter,
        beforeCount: filtered.length,
        sampleClientTypes: filtered.slice(0, 5).map(c => ({ id: c._id, clientType: c.clientType }))
      });
      filtered = filtered.filter(caso => caso.clientType === clientTypeFilter);
      console.log("[EXPEDIENTES][ClientType Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, clientType: c.clientType }))
      });
    }
    ...
    if (departmentFilter !== "all") {
      console.log("[EXPEDIENTES][Department Filter]:", {
        departmentFilter,
        beforeCount: filtered.length,
        sampleDepartments: filtered.slice(0, 5).map(c => ({ id: c._id, department: c.department })),
        allUniqueDepartments: [...new Set(filtered.map(c => c.department))].slice(0, 10)
      });
      filtered = filtered.filter(caso => {
      ...
      console.log("[EXPEDIENTES][Department Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, department: c.department }))
      });
    }

    // Filtro por jurisdicción/despacho
    if (jurisdictionFilter !== "all" && jurisdictionFilter.trim()) {
      console.log("[EXPEDIENTES][Jurisdiction Filter]:", {
        jurisdictionFilter,
        beforeCount: filtered.length,
        hasDespachoOptions,
        sampleJurisdictions: filtered.slice(0, 5).map(c => ({ id: c._id, office: c.office }))
      });
      
      if (hasDespachoOptions) {
        // Si hay opciones específicas, usar coincidencia exacta
        filtered = filtered.filter(caso => caso.office === jurisdictionFilter);
      } else {
        // Si no hay opciones específicas, usar búsqueda por texto
        const searchLower = jurisdictionFilter.toLowerCase().trim();
        filtered = filtered.filter(caso => 
          caso.office?.toLowerCase().includes(searchLower)
        );
      }
      
      console.log("[EXPEDIENTES][Jurisdiction Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, office: c.office }))
      });
    }
    ...
    if (processTypeFilter !== "all") {
      console.log("[EXPEDIENTES][ProcessType Filter]:", {
        processTypeFilter,
        beforeCount: filtered.length,
        sampleProcessTypes: filtered.slice(0, 5).map(c => ({ id: c._id, processType: c.processType }))
      });
      filtered = filtered.filter(caso => caso.processType === processTypeFilter);
      console.log("[EXPEDIENTES][ProcessType Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, processType: c.processType }))
      });
    }
    ...
    if (cityFilter !== "all") {
      console.log("[EXPEDIENTES][City Filter]:", {
        cityFilter,
        beforeCount: filtered.length,
        sampleCities: filtered.slice(0, 5).map(c => ({ id: c._id, city: c.city })),
      ...
      console.log("[EXPEDIENTES][City Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, city: c.city }))
      });
    }
    ...
    if (locationFilter !== "all") {
      console.log("[EXPEDIENTES][Location Filter]:", {
        locationFilter,
        beforeCount: filtered.length,
        sampleLocations: filtered.slice(0, 5).map(c => ({ id: c._id, location: c.location }))
      });
      filtered = filtered.filter(caso => caso.location === locationFilter);
      console.log("[EXPEDIENTES][Location Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, location: c.location }))
      });
    }
    ...
    if (typeFilter !== "all") {
      console.log("[EXPEDIENTES][Type Filter]:", {
        typeFilter,
        beforeCount: filtered.length,
        sampleTypes: filtered.slice(0, 5).map(c => ({ id: c._id, type: c.type }))
      });
      filtered = filtered.filter(caso => caso.type === typeFilter);
      console.log("[EXPEDIENTES][Type Filter Result]:", {
        afterCount: filtered.length,
        filteredSample: filtered.slice(0, 3).map(c => ({ id: c._id, type: c.type }))
      });
    }
    ...
    if (dateFrom) {
      ...
      filtered = filtered.filter(caso => {
        ...
        const isValid = !isNaN(casoDate.getTime()) && !isNaN(fromDate.getTime());
  ...
  console.log("[EXPEDIENTES][Table Data Debug]:", {
    ...
    currentPageCasos: currentPageCasos.map(c => ({ id: c._id, name: c.internalCode })),
    filteredCasosSample: filteredCasos.slice(0, 5).map(c => ({ id: c._id, name: c.internalCode }))
  });

  // Efecto para detectar automáticamente el final de los datos
  useEffect(() => {
    if (allLoadedCasos.length > 0 && !hasMoreToLoad) {
      const isCurrentPageIncomplete = currentPageCasos.length < itemsPerPage && currentPageCasos.length > 0;
      
      if (isCurrentPageIncomplete) {
        console.log("[EXPEDIENTES][End Detected]: Página actual incompleta, fin de paginación", {
          currentPage,
  ...
  }, [allLoadedCasos, currentPage, hasMoreToLoad, currentPageCasos.length, itemsPerPage])
  ...
  const estimatedTotalPages = totalRecords > 0 ? Math.ceil(totalRecords / itemsPerPage) : Math.max(totalLoadedPages, 1)
  
  // Detectar dinámicamente si hemos llegado al final
  const isLastPageIncomplete = allLoadedCasos.length > 0 && 
  ...
  const progressInfo = useMemo(() => {
    ...
    const safePercentage = actualTotal > 0 
  ...
  const handlePageChange = useCallback(async (page: number) => {
    ...
    if (requiredCasos > allLoadedCasos.length && hasMoreToLoad && !isLoadingMore) {
      console.log("[EXPEDIENTES][Auto Load]: Cargando más datos para página", page, {
        requiredCasos,
    ...
    setCurrentPage(page);
  }, [currentPage, itemsPerPage, allLoadedCasos.length, hasMoreToLoad, isLoadingMore, loadMoreFromServer]);
  ...
  const handleItemsPerPageChange = useCallback(async (newItemsPerPage: number) => {
    ...
    if (newItemsPerPage > allLoadedCasos.length && hasMoreToLoad && !isLoadingMore) {
      const batchSize = Math.max(newItemsPerPage, 20);
      await loadMoreFromServer(batchSize);
    }
  }, [itemsPerPage, allLoadedCasos.length, hasMoreToLoad, isLoadingMore, loadMoreFromServer]);

  // Obtener valores únicos para los filtros (memoizado)
  const uniqueValues = useMemo(() => ({
    estados: Array.from(new Set(allLoadedCasos.map(caso => caso.estado))),
    clientTypes: Array.from(new Set(allLoadedCasos.map(caso => caso.clientType))),
    departments: Array.from(new Set(allLoadedCasos.map(caso => caso.department))),
    jurisdictions: Array.from(new Set(allLoadedCasos.map(caso => caso.office))),
    processTypes: Array.from(new Set(allLoadedCasos.map(caso => caso.processType))),
    cities: Array.from(new Set(allLoadedCasos.map(caso => caso.city))),
    locations: Array.from(new Set(allLoadedCasos.map(caso => caso.location))),
      ...
      <div className="p-6">
        <Alert variant="destructive">
          <AlertDescription>
            <div className="space-y-4">
              <div>
                <p className="font-semibold">Error al cargar expedientes: {localError}</p>
    ...
    <div className="p-2 sm:p-4 md:p-6 min-w-0 overflow-x-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
        <div className="min-w-0 flex-1">
          <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold truncate">Gestión de Expedientes</h1>
          <p className="text-gray-600 mt-1 text-xs sm:text-sm">
                   ...
                   <span className={`ml-1 ${progressInfo.status === 'complete' ? 'text-green-600' : 'text-blue-600'}`}>
                     ({allLoadedCasos.length} de {Math.max(totalRecords, allLoadedCasos.length)} cargados)
                   </span>
                   {progressInfo.status === 'loading' && (
                     <span className="ml-1 sm:ml-2 text-xs bg-blue-100 text-blue-700 px-1 sm:px-2 py-0.5 sm:py-1 rounded-full">
      ...
      <Card>
        <CardHeader className="p-3 sm:p-4 md:p-6">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            ...
            >
              ...
              Filtros
              {(dateFrom || dateTo || estadoFilter !== "all" || clientTypeFilter !== "all" || departmentFilter !== "all") && (
                <span className="ml-1 sm:ml-2 text-xs px-1 py-0 bg-red-100 text-red-800 rounded">
                  {[dateFrom, dateTo, estadoFilter !== "all" ? "1" : "", clientTypeFilter !== "all" ? "1" : "", departmentFilter !== "all" ? "1" : ""].filter(Boolean).length}
                </span>
              )}
            </Button>
          </div>
            ...
            <CollapsibleContent className="space-y-4 mt-4 p-2 sm:p-4 border rounded-lg bg-gray-50">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-4">
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Código Interno</label>
                  <Input
                ...
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Fecha desde</label>
                  <DatePicker
                    selected={dateFrom}
                ...
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Fecha hasta</label>
                  <DatePicker
                    selected={dateTo}
                ...
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <Select value={estadoFilter} onValueChange={setEstadoFilter}>
                    ...
                    <SelectContent>
                      ...
                      {uniqueValues.estados.filter(estado => estado && estado.trim() !== '').map(estado => (
                ...
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Tipo de Cliente</label>
                  <Select value={clientTypeFilter} onValueChange={setClientTypeFilter}>
                    ...
                    <SelectContent>
                      ...
                      {uniqueValues.clientTypes.filter(type => type && type.trim() !== '').map(type => (
                ...
                {/* Filtro por Tipo de Proceso */}
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Tipo de Proceso</label>
                  <Select value={processTypeFilter} onValueChange={setProcessTypeFilter}>
                    ...
                    <SelectContent>
                      ...
                      {uniqueValues.processTypes.filter(processType => processType && processType.trim() !== '').map(processType => (
                ...
                {/* Filtro por Departamento */}
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Departamento</label>
                  <Select value={departmentFilter} onValueChange={setDepartmentFilter}>
                    ...
                    <SelectContent>
                      ...
                      {departments.map(dept => (
                        <SelectItem key={dept.codigo} value={dept.nombre}>{dept.nombre}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                {/* Filtro por Ciudad */}
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">Ciudad</label>
                  <Select 
                    value={cityFilter} 
                  ...
                  >
                    <SelectTrigger className="w-full border-gray-300 text-sm">
                      <SelectValue placeholder={departmentFilter === "all" ? "Selecciona un departamento" : "Todas"} />
                    </SelectTrigger>
                    <SelectContent className="max-h-60">
                      <SelectItem value="all">Todas las ciudades</SelectItem>
                      {filteredCities.map((city, index) => (
                        <SelectItem key={`${city.codigo}-${city.departamento}-${index}`} value={city.nombre}>
                          {city.nombre}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div className="space-y-1 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-medium text-gray-700">
                    ...
                    {cityFilter !== "all" && (
                      <span className="text-xs text-gray-500 ml-1">
                        ({hasDespachoOptions ? availableDespachos.length : 'búsqueda libre'} opciones)
                      </span>
                    )}
                  </label>
                  ...
                  {hasDespachoOptions && cityFilter !== "all" ? (
                    ...
                    >
                      <SelectTrigger className="w-full border-gray-300 text-sm">
                        <SelectValue placeholder={cityFilter === "all" ? "Selecciona una ciudad primero" : "Todos los despachos"} />
                      </SelectTrigger>
                      <SelectContent className="max-h-60">
                        <SelectItem value="all">Todos los despachos</SelectItem>
                        {availableDespachos.map((despacho, index) => (
                          <SelectItem key={`${despacho}-${index}`} value={despacho}>
                            {despacho}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    <Input
                      ...
                      onChange={(e) => setJurisdictionFilter(e.target.value || "all")}
                      placeholder={cityFilter === "all" ? "Selecciona una ciudad primero" : "Buscar despacho..."}
                      disabled={cityFilter === "all"}
                      className="text-sm"
                    />
                  )}
                ...
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <Select value={typeFilter} onValueChange={setTypeFilter}>
                    ...
                    <SelectContent>
                      <SelectItem value="all">Todos los tipos</SelectItem>
                      {uniqueValues.types.filter(type => type && type.trim() !== '').map(type => (
                        <SelectItem key={type!} value={type!}>{type}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 pt-2 border-t">
                <div className="text-xs sm:text-sm text-gray-600">
                  Página {currentPage} • {filteredCasos.length} de {allLoadedCasos.length} expedientes
                  {filteredCasos.length !== allLoadedCasos.length && (
                    <span className="ml-1 text-pink-600">(filtrados)</span>
                  )}
                  {hasMoreToLoad && (
                    <span className="ml-2 text-blue-600 cursor-pointer" onClick={() => loadMoreFromServer()}>
                      (Cargar más)
                    </span>
                  )}
                </div>
        ...
        <CardContent className="bg-white p-3 sm:p-4 md:p-6">
            ...
            <Table className="bg-white">
              <TableHeader className="bg-pink-600">
                <TableRow className="bg-pink-600 hover:bg-pink-600">
                  ...
                  <TableHead className="text-white font-semibold">Radicado</TableHead>
                  <TableHead className="text-white font-semibold">Ubicación Expediente</TableHead>
                  <TableHead className="text-white font-semibold">Despacho</TableHead>
                  <TableHead className="text-white font-semibold">Ciudad</TableHead>
                  <TableHead className="text-white font-semibold">Tipo de Cliente</TableHead>
                  <TableHead className="text-white font-semibold">Activo</TableHead>
                  <TableHead className="text-white font-semibold">Estado</TableHead>
                  <TableHead className="text-white font-semibold">Actualizado</TableHead>
          ...
          <div className="lg:hidden space-y-3">
            {currentPageCasos.length > 0 ? (
              currentPageCasos.map((caso) => (
                <div key={caso._id} className="bg-white border rounded-lg p-3 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-gray-900 truncate text-sm">
                        {caso.proceduralParts[0]?.name || 'Sin nombre'}
                      </h3>
                      <p className="text-xs text-gray-600">#{caso.internalCode}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <p className="font-medium truncate">{caso.settled || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Ciudad:</span>
                      <p className="font-medium truncate">{caso.city || 'N/A'}</p>
                    </div>
                    <div>
                  ...
                  <div className="mt-2 pt-2 border-t">
                    <div className="flex justify-between items-center text-xs text-gray-500">
                      <span className="truncate">Actualizado: {caso.createdAt ? new Date(caso.createdAt).toLocaleDateString('es-ES') : 'N/A'}</span>
                      <span className={`px-1 py-0.5 rounded-full text-xs font-medium ${
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4 mt-4 p-2 sm:p-4 border-t overflow-x-auto">
              <div className="flex items-center gap-1 sm:gap-2 min-w-0">
                <span className="text-xs sm:text-sm text-gray-600 whitespace-nowrap">
                  Mostrando {startIndex + 1} - {Math.min(endIndex, allLoadedCasos.length)} de {totalRecords} expedientes
                  {/* <span className={`ml-1 ${progressInfo.status === 'complete' ? 'text-green-600' : 'text-blue-600'}`}>
                        ...
                        pages.push(
                          <Button
                            ...
                            className={`w-6 h-6 sm:w-8 sm:h-8 p-0 text-xs sm:text-sm ${
                              !pageHasData ? 'text-blue-600 border-blue-300' : ''
                            }`}
                            onClick={() => handlePageChange(i)}
                            disabled={isLoadingMore && !pageHasData}
                          >
                            {isLoadingMore && !pageHasData && i === currentPage + 1 ? (
                    ...
                    return (
                      <Button
                        ...
                        size="sm"
                            ...
                            disabled={isLoadingMore && !pageHasData}
                          >
                            {isLoadingMore && !pageHasData && pageNum === currentPage + 1 ? (
                              <Loader2 className="h-3 w-3 animate-spin" />
                            ) : (
                              pageNum
                            )}
