...
export default function EquipoView() {
  ...
  const [teamRepository] = useState(() =>
    container.get<TeamRepository>("TeamRepository")
  );

  const roles = ALL_SELECTABLE_ROLES.map((role) => ({
    value: role,
  ...
  const getMemberRole = (roles?: string[]) => {
    const role =
      roles && roles.length > 0
        ? getSelectableRoleFromArray(roles)
        : "Sin rol asignado";
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-gray-600 mt-1 text-xs sm:text-sm">
            Administra el equipo de trabajo y colaboradores
          </p>
        </div>
        <Button
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="font-medium">
                              {getMemberFullName(member.name, member.lastname)}
                            </div>
                          </div>
                        </div>
                      </TableCell>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center space-x-2 min-w-0 flex-1">
                      <Avatar className="w-8 h-8">
                        <AvatarImage
                          src=""
                          alt={getMemberFullName(member.name, member.lastname)}
                        />
                        <AvatarFallback>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-4">
              <Avatar className="w-16 h-16">
                <AvatarImage
                  src=""
                  alt={getMemberFullName(editForm.name, editForm.lastname)}
                />
            ...
            {isEditMode && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    value={getSelectableRoleFromArray(editForm.roles)}
                    onValueChange={(value) =>
                      setEditForm({ ...editForm, roles: getRolesArray(value) })
                    }
                  >
                    <SelectTrigger>
          ...
          <DialogFooter>
            {isEditMode ? (
              <>
                ...
                <Button onClick={handleSaveMember}>Guardar Cambios</Button>
              </>
            ) : (
              <>
                <Button variant="outline" onClick={handleCloseModal}>
